...
class Template(yaml.YAMLObject):
    ...
    def my_apply_2(self, example, truncate=True, highlight_variables=False):
                    ...
                    if type(post_parent) == list:
                        ...
                        for parent_key in dict(post_parent).keys():
                            if post_parent[parent_key] == post_item:
                                ...
                                if key in ['label','star', 'score', 'span1_index', 'span2_index', 'idx'] or type(post_item)==bool:
        ...
        while(len(if_0)>0 or len(for_0)>0):
            if len(if_0)>0 and if_0[0]==cnt:
                prompt+="{% endif %}"
                if_0.pop(0)
            elif len(for_0)>0 and for_0[0]==cnt:
                prompt+="{% endfor %}"
                for_0.pop(0)
            cnt+=1
        ...
        #print("protected_example", protected_example)
        input = {}
        import copy
        ...
        for key in protected_example.keys():
            #print(key, value)
            if key!="choice1" and key!="choice2" and key!="answer_choices" and key!="option1" and key!="option2" and key!="metadata" and key!="meta":
        ...
        for i in input.keys():
            if "/{/{"+i+"/}/}" in rendered_example:
                output+= (i + ": " + input[i] + "\n")
        return output, re.sub('^\W*/\{/\{[^\}]+/\}/\}|/\{/\{[^\}]+/\}/\}\W*$|/\{/\{|/\}/\}', '', rendered_example)
